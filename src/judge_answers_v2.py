...
DEFAULT_ANSWERS_INPUT = Path("data/answers/answers_vector_v2_prompt_grounded.jsonl")
DEFAULT_CHUNKS_INPUT = Path("data/chunks/chunks.jsonl")
DEFAULT_OUTPUT = Path("data/answers/answers_vector_v2_prompt_grounded_judged.jsonl")
...
class AnswerEvaluation(BaseModel):
    reasoning: str = Field(
        description=(
            ...
            "requested by the question, and avoids major unsupported claims."
        )
    )
...
- Be careful about question-target completeness:
  - If the question asks for specific resources, named frameworks, named organisations, named documents, or explicit examples, then the answer should include those specific items, not just vague generalities.
  - If the ground-truth passage contains concrete named items that are central to answering the question, omitting them can make the answer incomplete.
- Prefer judging against what the question actually asks for, not just whether the answer sounds broadly related.
...
- Do require important named items when the question explicitly asks for specific resources, examples, or concrete items.
