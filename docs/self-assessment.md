...
| Retrieval flow      | Good | Knowledge base and LLM‑related flow implemented. |
| Retrieval evaluation | Good | Text vs vector retrieval evaluated; vector chosen on evidence. |
| LLM evaluation      | Poor | LLM used in supporting workflows, but final answers not evaluated. |
...
| Hybrid search       | — | Text/vector baselines exist; hybrid not implemented yet. |
...
The project uses both a knowledge base and LLM‑related components in its flow. ACSC documents are downloaded, cleaned, chunked, stored as `data/chunks/chunks.jsonl`, and loaded into a PostgreSQL `chunks` table. Retrieval scripts implement full‑text search and pgvector‑based dense retrieval, and there is an LLM helper to support evaluation pipelines and planned answer generation. This meets the “2 points / Good” definition: both a knowledge base and an LLM are part of the overall flow, not just direct prompting. Evidence is in `src/db_init.py`, `src/db_load_chunks.py`, `src/retrieve_text.py`, `src/retrieve_vector.py`, and the corpus files.

## Retrieval evaluation — (Good)

Retrieval quality is evaluated explicitly, and multiple retrieval approaches are compared. The project builds a synthetic ground‑truth set and uses `src/evaluate_retrieval.py` to compare text retrieval and vector retrieval on the same questions, reporting metrics such as hit rate and MRR. Current results show that vector retrieval substantially outperforms the text baseline, so the preferred retriever is chosen based on evidence. This matches the “2 points / Good” definition: multiple retrieval approaches are evaluated and the best one is used. Evidence is in `src/evaluate_retrieval.py`, `data/ground_truth_synthetic.jsonl`, and the recorded metrics.
...
Hybrid search (combining text and vector retrieval) has not been implemented yet. The project now has both baselines, so this is a natural next step for retrieval experimentation once the current vector baseline is stable.

### Document re-ranking

Document or chunk reranking is not yet present. The current system relies on the primary retriever’s ranking only. A second‑stage reranker could be added later to refine results, especially in a hybrid setup.
...
- Monitoring (Poor)
- Containerization (Poor)
- Bonus implementation categories (not yet implemented)
...
- Experiment with hybrid search, now that both text and vector baselines exist.
