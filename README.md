...
- evaluated text, vector, and hybrid retrieval
- evaluated answer-generation variants with an LLM-as-judge layer
...
14. Add the application or interface layer (e.g. Streamlit UI and monitoring).
...
- **AI Navigator**:
  ...
  - vector retrieval as the default backend (`top_k` configurable)
...
- evaluated text, vector, and hybrid retrieval over a synthetic benchmark
- evaluated answer-generation variants with an LLM-as-a-judge layer
- a selected default path: vector retrieval + v2 prompt-grounded answers
...
- targeted retrieval enhancements such as query rewriting or reranking, evaluated against the existing benchmark
